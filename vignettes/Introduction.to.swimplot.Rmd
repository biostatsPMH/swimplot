...
This vignette goes through an example to create swimmers plots, and demonstrates converting a dataframe to the required format.
...
- A column name for the fill and colour (outline of the bars) can also be included
- Individual bars can change colour over time
...
The `swimmer_plot()` function includes the option to have the bars change colours. Each section of the bars should be in a different row, where each row includes the time that section ends. By default the bars are plotted in increasing order, the order can also be set to "decreasing", by a column in the dataframe (and then increasing order), or any desired order. Here the bars are ordered by the starting treatment, and follow up time. 
\tiny
...
Axis and legends can be labeled with the layer `labs()`, and the y axis can be removed when patient identifiers are not required. 

```{r axis, echo=T, message=FALSE, warning=FALSE}
