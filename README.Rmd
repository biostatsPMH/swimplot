...
**swimplot** is a package that works with **ggplot2** to create swimmer plots. 
...
You can install the main version of **swimplot** from [GitHub](https://github.com/) with:
...
An alternative version of **swimplot** which allows for patterns in the bars (eg. stripes) can be installed from [GitHub](https://github.com/) with:
...
```{r example1a, message=F, warning=F}
library(ggplot2);
library(swimplot);
data("arm", "res", "ae");
...
```{r example1b, echo=FALSE, out.width='100%'}
...
```{r example2a, message=F, warning=F}
library(ggplot2);
library(swimplot);
data("arm", "res", "ae");
...
```{r example2b, echo=FALSE, out.width='100%'}
